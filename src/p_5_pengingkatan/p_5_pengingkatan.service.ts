...
import { P_5_peningkatanModel } from "./p_5_pengingkatan.model";
...
@Injectable()
export class P5PengingkatanService {
  constructor(
    @InjectModel("p_5_peningkatan")
    private readonly localModel: Model<P_5_peningkatanModel>
  ) {
  }
  ...
  async getKodeData() {
    ...
    return `P_5_Peningkatan_${data + 1}`;
  }

  // Post / createData
  async createData(
    kode_data: string,
    payload: Create_p_5_peningkatanDto
  ) {
  ...
  async editData(
    id: string,
    payload: Update_p_5_peningkatanDto
  ) {
    try {
