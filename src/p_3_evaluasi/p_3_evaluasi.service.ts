...
import { P_3_evaluasiModel } from "./p_3_evaluasi.model";
...
@Injectable()
export class P3EvaluasiService {
  constructor(
    @InjectModel("p_3_evaluasi")
    private readonly localModel: Model<P_3_evaluasiModel>
  ) {
  }
  ...
  async getKodeData() {
    ...
    return `P_3_Evaluasi_${data + 1}`;
  }

  // Post / createData
  async createData(
    kode_data: string,
    payload: Create_p_3_evaluasiDto
  ) {
  ...
  async editData(
    id: string,
    payload: Update_p_3_evaluasiDto
  ) {
    try {
